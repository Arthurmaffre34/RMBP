...
def trainDDPG():
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            action = (action + 1) / 2 + np.random.normal(0, exploration_noise, size=4)
            ...
            if len(replay_buffer) > start_training_after:
                ...
                critic_loss_m.append(critic_loss.detach().cpu())
                #print(critic_loss_m)
                actor_loss_m.append(actor_loss.detach().cpu())
        ...
        actor_loss_m = np.array(actor_loss_m)
        critic_loss_m = np.array(critic_loss_m)
        print(critic_loss_m.mean())
        # Log reward in TensorBoard
        writer.add_scalar("Episode Reward", episode_reward, episode)
        writer.add_scalar('Actor loss', critic_loss_m.mean(), episode)
        writer.add_scalar('Critic loss', actor_loss_m.mean(), episode)
...
'''
            ...
            if keyboard.is_pressed('p'):
                print("pause veuillez appuyer sur entrer pour continuer")
                input()

            if keyboard.is_pressed('e'):
                if input("fermeture du programme sans sauvegarde êtes vous sur? O ou N").lower == 'o':
                    writer.close()
                    exit()
'''